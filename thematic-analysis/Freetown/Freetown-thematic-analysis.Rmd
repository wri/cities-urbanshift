...
```{r warning=FALSE, message=FALSE, echo = FALSE,  eval = TRUE}
# read processed boundary data
planning_area <- st_read("./data/processed/planning_area.geojson",
                         quiet = TRUE)
...
# read urban expansion stat
city_subregion_urban_expansion = read.csv("./output/planning_area_urban_expansion.csv")

# print(city_subregion_urban_expansion)
...
# print(head(city_subregion_urban_expansion_map))
...
## Urban expansion by Land use class
...
# plot the map
leaflet(city_land_use, height = 500, width = "100%") %>%
  addTiles() %>%
  addProviderTiles("OpenStreetMap.France", group = "OSM") %>%
  addProviderTiles(providers$CartoDB.DarkMatter , group = "CartoDB") %>%
...
# read data
city_subregion_urban_expansion = read.csv("./output/land_use_urban_expansion.csv",
                                          row.names = 1)
# classify
city_subregion_urban_expansion = city_subregion_urban_expansion %>% 
...
- [Google Earth Engine script used for this analysis](https://code.earthengine.google.com/d14d537e1d3e9fed52fb30287a5c19e9)
