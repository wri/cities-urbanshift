...
activities and needs associated with the Freetown Structure Plan and the Mayor’s Transform Freetown 
...
$$Accuracy = \frac{NbCorresctPrediction}{TotalPrediction}$$
Where: 

- *NbCorresctPrediction = TP (True Positive)+ TN (True Negative)*

- *NbCorresctPrediction = TP (True Positive)+ TN (True Negative) + FP (False Positive) + FN (False Negative)*
...
collect.land.class.by.year = function(year){
...
# collect NDVI for year 2020

classified_map_2016 = collect.land.class.by.year(year ="2016")
...
classified_map_2021 = collect.land.class.by.year(year ="2021")
...
## Urban expansion by city

The following analysis providesan  urban expansion indicator computed at sub-regional level in order to identify localities presenting the highest increase in urban areas. 

Localities with higher urban expansion during the six last years are: `Gloucester`, `Mamba Ridge II`, and `Thunderhill` with urban an expansion ratio of respectively `41.75%`, `26%`, and  `24.65%`.

We observe a cluster of localities in the north-east of Freetown with a high urban expansion regrouping `Gloucester`, `Mamba Ridge II`, `Thunderhill`, `Bathurst`, and `Leicester`.

```{r warning=FALSE, message=FALSE, echo = FALSE,  eval = TRUE}

# collect sub-boundries date

# define path
city_sub_boundary_path = paste("https://storage.googleapis.com/urbanshift/freetown/boundaries/SLE-Freetown_sub_region.geojson")

# read the data#
city_sub_boundary <- st_read(city_sub_boundary_path,
                         quiet = TRUE)
...
leaflet(height = 500, width = "100%") %>%
  ...
  #             group = "Administrative boundaries",
  #             fillColor = ~pal_subRegion(Urban),
  #             stroke = TRUE, 
  #             color = col_BoldEarthGrey, 
  #             weight = 3,
  #             dashArray = "3",
  #             smoothFactor = 0.3, 
  #             fillOpacity = 1,
  ...
  #               fillOpacity = 0.3,
  #               bringToFront = TRUE),
  #             label = city_sub_boundary$name,
  #             labelOptions = labelOptions(
  #               style = list("font-weight" = "normal", padding = "3px 8px"),
  #               textsize = "15px",
  #               direction = "auto")) %>%
  # addLegend(pal = pal_subRegion, 
  #           values = test$Urban, 
  #           opacity = 0.9, 
  #           title = "Urban area",
  #           position = "bottomright",
...
city_subregion_urban_expansion %>% 
  ...
  add_trace(x = ~factor(name), 
            ...
            type = "bar",
            orientation = "v")  %>% 
  layout(title = "Top cities with highest urban expansion",
...
We proposed an urban expansion indicator based on the percentage of urban area between between 2016 and 2021. Aggregated at the locality level, the proposed indicator highlighted the cities that present the highest urban expansion, such as `Gloucester` (more than 40% of urban areas in the last 6 years).
